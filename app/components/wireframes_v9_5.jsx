...
const STATES = {
  ...
  // [v9.5] 룸 설정: draft 상태(공개 전, 초대+세팅만)와 public 상태(운영 중) 두 가지 시연
  "room-settings": ["draft 상태", "public 상태"],
...
// ===== [v9.5] 룸 설정 — 라이프사이클 전환 UI, 비공개 룸 제거 =====
// state: "draft 상태" | "public 상태"
function RoomSettingsScreen({ state }) {
  ...
  return (<div>
    ...
    <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {/* 라이프사이클 상태 표시 */}
      <div style={{ padding: "12px 14px", background: isDraft ? "#FAEEDA" : "#E1F5EE", borderRadius: 12, border: `1.5px solid ${isDraft ? "#EF9F27" : "#5DCAA5"}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
          <div style={{ width: 8, height: 8, borderRadius: "50%", background: isDraft ? "#854F0B" : "#0F6E56" }} />
          <span style={{ fontSize: 13, fontWeight: 700, color: isDraft ? "#854F0B" : "#0F6E56" }}>{isDraft ? "Draft — 준비 중" : "Public — 운영 중"}</span>
        </div>
        <div style={{ fontSize: 11, color: isDraft ? "#854F0B" : "#0F6E56", lineHeight: 1.5 }}>
          {isDraft
            ? "이 룸은 아직 외부에 공개되지 않았습니다. 룸 정보를 채우고 멤버를 초대할 수 있습니다. 이슈 발행은 공개 후 가능합니다."
      ...
      {/* 1:1 초대 링크 — draft에서도 노출(멤버 모집용), public에서는 모집 닫힘일 때만 */}
      ...
      {isDraft ? (
        <div>
          ...
          <div style={{ fontSize: 11, color: "#5F5E5A", marginTop: 8, lineHeight: 1.5, textAlign: "center" }}>이 룸이 룸 목록과 피드에 노출되고, 이슈를 발행할 수 있게 됩니다. 공개 후에는 되돌릴 수 없습니다.</div>
        </div>
      ) : (
        <div>
          ...
          <div style={{ padding: "12px 0", background: "#FCEBEB", color: "#993C1D", borderRadius: 12, fontSize: 14, fontWeight: 600, textAlign: "center" }}>운영 종료</div>
          <div style={{ fontSize: 11, color: "#B4B2A9", marginTop: 8, lineHeight: 1.5, textAlign: "center" }}>새 이슈 발행과 멤버 가입이 중단됩니다. 기존 이슈와 댓글은 운영자와 멤버에게만 남습니다. 종료 후에는 되돌릴 수 없습니다.</div>
        </div>
      )}
    </div>
  </div>);
...
const SCREEN_INFO = {
  ...
  "room-settings": { route: "/operator/rooms/:id/settings", purpose: "운영자가 룸을 관리", data: ["라이프사이클 상태(draft/public/archived)", "룸 정보", "이 룸의 시선(Aspect, 필수)", "멤버 모집(public일 때만)", "1:1 초대 링크", "멤버 목록(Aspect 포함)"], cta: ["룸 공개(draft→public)", "운영 종료(public→archived)", "저장", "1:1 링크 생성"], notes: "[v9.5] 라이프사이클 3단계: draft(준비중)→public(운영중)→archived(종료). 단방향 전이. 상단에 현재 상태 배너. draft일 때: 룸 세팅+초대만 가능, 모집 토글 없음, 하단에 '룸 공개' 버튼+설명. public일 때: 모집 토글 노출, 하단에 '저장'+' 운영 종료' 버튼+설명. '공개 상태(비공개/공개)' 토글 삭제 — 비공개 운영 룸은 MVP에서 의도적 제외." },
