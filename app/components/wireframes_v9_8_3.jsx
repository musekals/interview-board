...
function Comment({ nick, aspect, text, upvotes, time, replies, isRep, isOperator, menuOpen }) {
  return (<div style={{ padding: "12px 16px", borderBottom: "1px solid #EEEDEA", position: "relative" }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        {/* [v9.8.3] 운영자에게만 노출되는 ⋯ 메뉴. 대표 댓글 지정/해제의 진입점. */}
        {isOperator && <span style={{ fontSize: 14, color: "#888780", cursor: "pointer", padding: "0 2px", lineHeight: 1 }}>⋯</span>}
      </div>
    </div>
    {/* [v9.8.3] ⋯ 메뉴 '열린 상태' 시연 — 실제 구현에선 클릭 시 노출.
                지정/해제는 isRep 값에 따라 라벨 전환. */}
    {isOperator && menuOpen && (
      <div style={{ position: "absolute", top: 32, right: 16, background: "#fff", border: "1px solid #D3D1C7", borderRadius: 10, boxShadow: "0 4px 12px rgba(0,0,0,0.08)", minWidth: 140, zIndex: 5 }}>
        <div style={{ padding: "10px 14px", fontSize: 13, color: isRep ? "#993C1D" : "#534AB7", fontWeight: 500, cursor: "pointer" }}>
          {isRep ? "대표 해제" : "대표로 지정"}
        </div>
      </div>
    )}
    ...
    {replies && replies.length > 0 && (<div style={{ marginTop: 8, marginLeft: 16, borderLeft: "2px solid #EEEDEA", paddingLeft: 12 }}>{replies.map((r, i) => (<div key={i} style={{ paddingBottom: 8, marginBottom: i < replies.length - 1 ? 8 : 0, borderBottom: i < replies.length - 1 ? "1px solid #F1EFE8" : "none" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontSize: 11, color: "#B4B2A9" }}>⚑</span>
          {/* [v9.8.3] 답글에도 운영자 ⋯ 메뉴 노출 — 답글도 대표로 지정할 수 있다는 전제. */}
          {isOperator && <span style={{ fontSize: 12, color: "#B4B2A9", cursor: "pointer", padding: "0 2px", lineHeight: 1 }}>⋯</span>}
        </div>
      </div>
...
function IssueDetailScreen({ state }) {
  ...
  return (<div>
    ...
    {/* [v9.8.3] 운영자(Member/Operator state)에게만 각 댓글의 ⋯ 메뉴 노출.
                대표 댓글 지정은 별도 페이지가 아니라 여기에서 인라인으로 수행.
                첫 번째 댓글만 '메뉴 열린 상태'로 시연. */}
    <Comment nick="@개발자L" aspect="개발자" text="구현 관점에서 streaming 처리와 tool use 핸들링이 가장 까다로웠습니다. 특히 중간 결과를 사용자에게 어떻게 보여줄지가 핵심." upvotes={12} time="2시간 전" isOperator={canComment} menuOpen={canComment} replies={[{ nick: "@김PM", aspect: "PM", text: "동의합니다. 중간 상태 노출이 신뢰도에 직결되더라고요.", upvotes: 5, time: "1시간 전" }]} />
    <Comment nick="@PM박" aspect="PM" text="저희는 리서치 요약부터 시작했는데, 사용자가 결과를 검토하고 수정할 수 있는 지점을 명확히 둔 게 컸습니다." upvotes={8} time="3시간 전" isOperator={canComment} replies={[]} />
...
function RoomSettingsScreen({ state }) {
  ...
  return (<div>
    ...
    <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {isDraft && (
        <div>
          {/* [v9.8.3] 보조 CTA: 룸 초안 저장 — 공개 전에 수정 사항만 저장하고 싶을 때 */}
          <div style={{ padding: "12px 0", background: "#fff", color: "#534AB7", border: "1.5px solid #534AB7", borderRadius: 12, fontSize: 14, fontWeight: 600, textAlign: "center", marginBottom: 6 }}>룸 초안 저장</div>
          <div style={{ fontSize: 11, color: "#888780", marginBottom: 14, lineHeight: 1.5, textAlign: "center" }}>룸을 공개하지는 않고, 수정 사항만 저장합니다.</div>
...
const SCREEN_INFO = {
  ...
  issue: { route: "/rooms/:slug/issues/:id", purpose: "이슈 본문 + 대표 댓글 + 전체 댓글", data: ["이슈 전체", "룸 Aspect", "대표 댓글(작성자 Aspect)", "전체 댓글(작성자 Aspect)", "운영자 전용: 각 댓글 우측 ⋯ 메뉴"], cta: ["업보트", "스크랩", "댓글 작성", "참여하기", "대표 지정/해제(운영자, ⋯ 메뉴)"], notes: "[v9.6] 댓글 입력 영역 4분기로 축소: Member(작성 가능), Subscriber(Aspect 보유/미보유 통합 — 같은 카피/같은 레이아웃, 버튼 라벨만 전환), Subscriber(모집 닫힘), Visitor. 모든 댓글에 작성자 Aspect 칩. Subscriber 통합 카피: '이 룸은 다음 정체성을 가진 사람이 댓글을 작성합니다. 보유하고 있다면 바로 멤버가 되고, 없다면 정체성을 추가한 뒤 참여할 수 있어요.' [v9.8.3] 대표 댓글 지정이 별도 페이지에서 이슈 상세로 이동. 운영자에게만 각 댓글 우측에 ⋯ 메뉴가 노출되고, 여기서 '대표로 지정' / '대표 해제'를 수행. 이미 대표인 댓글은 '대표 해제'로 라벨 전환. 답글도 동일하게 지정 가능." },
