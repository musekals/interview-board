...
function StatusBadge({ status }) {
  ...
  if (status === "예정") return <Badge text="예정" color="blue" />;
  if (status === "종료") return <Badge text="종료" color="gray" />;
  return null;
}

function SH({ children }) {
  return <div style={{ fontSize: 13, fontWeight: 600, color: "#5F5E5A", padding: "12px 16px 6px", background: "#FAFAF8" }}>{children}</div>;
}

function RoomThumb({ size, color }) {
  const sz = size || 36;
  return <div style={{ width: sz, height: sz, borderRadius: sz > 30 ? 12 : 8, background: color || "#EEEDFE", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", flexShrink: 0 }}><div style={{ width: "100%", height: "100%", background: `linear-gradient(135deg, ${color || "#EEEDFE"} 60%, #D3D1C7 100%)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: sz * 0.38, color: "#534AB7", fontWeight: 600 }}>◈</div></div>;
}
...
function FeedScreen({ state }) {
  ...
  return (
    <div>
      ...
      <BigIssueCard room="리벨북스 비문학 스터디" title="이번 장의 핵심 질문은 무엇인가" opLine="저자 결론보다 문제제기에 집중합시다" body="저자가 3장에서 제기하는 문제의식은 결론 부분보다 오히려 초반의 질문 자체에 더 큰 무게가 있습니다. 독자가 스스로 답을 구성해야 하는 구조입니다." repNick="@닉E" repComment="오히려 이 대목이 저자의 핵심 논점입니다. 결론에서 말하는 것보다 3장 초반에 던진 질문의 구조가 이 책 전체를 관통하고 있습니다." status="종료" comments={18} upvotes={35} views={198} hasImage={false} />
    </div>
  );
}
...
function IssueListScreen({ state }) {
  ...
  return (
    <div>
      ...
      <CompactIssueCard room="나는 솔로 해석 룸" title="다음 회차 예고에서 가장 궁금한 점" opLine="예고편 편집이 핵심" repShort="" status="예정" comments={8} upvotes={19} views={95} />
    </div>
  );
}
    ...
    <div>
      ...
      {rooms.map((r, i) => (
        <div key={i} style={{ padding: "14px 16px", borderBottom: "1px solid #EEEDEA" }}>
          ...
          <div style={{ padding: "8px 0", background: "#534AB7", color: "#fff", borderRadius: 8, fontSize: 13, fontWeight: 600, textAlign: "center" }}>{state === "Subscriber" ? "구독 중 ✓" : "구독하기"}</div>
        </div>
      ))}
    </div>
  );
...
function RoomScreen({ state }) {
  ...
  const roomName = isExpansion ? "나는 솔로 해석 룸" : (isMember ? "나는 솔로 해석 룸" : "리벨북스 비문학 스터디");
  const roomDesc = isExpansion ? "감정보다 맥락과 편집으로 봅니다" : (isMember ? "감정보다 맥락과 편집으로 봅니다" : "저자의 질문을 함께 읽습니다");
  return (
    <div>
      ...
      <div style={{ padding: "16px 16px 12px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
              <span style={{ fontSize: 19, fontWeight: 700, color: "#2C2C2A" }}>{roomName}</span>
            </div>
        ...
        <div style={{ fontSize: 13, color: "#5F5E5A", marginBottom: 10 }}>{roomDesc}</div>

        {/* 멤버: 소개+대표반응 접기 */}
        {isMember ? (
          <div style={{ fontSize: 12, color: "#B4B2A9", padding: "6px 0", marginBottom: 8, cursor: "pointer", display: "flex", gap: 12 }}>
            <span>▸ 룸 소개 보기</span>
            <span>▸ 대표 반응 보기</span>
          </div>
        ) : (
          <div style={{ fontSize: 13, color: "#444441", background: "#F9F9F6", padding: "12px 14px", borderRadius: 10, lineHeight: 1.6, marginBottom: 12, whiteSpace: "pre-line" }}>
            ...
            {isShowcase ? "• 한 챕터씩 깊이 읽고 저자의 문제의식을 곱씹습니다\n• 이 방은 읽기 중심으로 운영됩니다" : "• 장면의 의도와 편집 포인트를 같이 봅니다\n• 한 줄 반응보다 이유 있는 댓글을 중시합니다"}
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {isExpansion && !isOp && <div style={{ width: 40, height: 40, borderRadius: 10, border: "1px solid #D3D1C7", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, color: "#888780", cursor: "pointer", flexShrink: 0 }}>⋯</div>}
        </div>
        {isExpansion && !isOp && <div style={{ fontSize: 11, color: "#0F6E56", marginTop: 6, textAlign: "right" }}>{isMember ? "⋯ → 댓글 멤버 탈퇴하기" : "같이 이야기하고 싶다면 →"}</div>}
        {isShowcase && !isMember && !isOp && <div style={{ fontSize: 11, color: "#B4B2A9", marginTop: 6, textAlign: "center", fontStyle: "italic" }}>이 방은 읽기 중심으로 운영됩니다</div>}
      </div>
      ...
      {isMember ? (
        ...
        <>
          ...
          <div style={{ padding: "0 16px 8px" }}>
            {[{ nick: "@닉A", text: isShowcase ? "3장 핵심은 결론이 아니라 저자가 던진 질문의 구조에 있습니다" : "이 장면은 호감 표현이 아니라 편집 의도가 드러나는 순간", issue: isShowcase ? "이번 장의 핵심 질문" : "이번 선택, 진짜 의도?" }, { nick: "@닉B", text: isShowcase ? "오히려 4장에서 이 논점이 다시 뒤집히는 구조가 인상적" : "편집이 그렇게 보이게 한 것. 실제 의도와는 다를 수도", issue: isShowcase ? "4장 반전 구조 분석" : "데이트 선택, 어떻게 읽히나" }].map((c, i) => (
              <div key={i} style={{ padding: "10px 12px", background: "#F9F9F6", borderRadius: 8, borderLeft: "3px solid #AFA9EC", marginBottom: 6 }}><div style={{ fontSize: 12, fontWeight: 600, color: "#534AB7", marginBottom: 2 }}>{c.nick}</div><div style={{ fontSize: 13, color: "#2C2C2A", lineHeight: 1.45 }}>{c.text}</div><div style={{ fontSize: 11, color: "#B4B2A9", marginTop: 4 }}>→ {c.issue}</div></div>
            ))}
          </div>
          <SH>대표 이슈</SH>
          <div style={{ margin: "0 16px 8px", padding: 14, border: "1.5px solid #AFA9EC", borderRadius: 12, background: "#FCFCFA" }}>
            ...
            <div style={{ fontSize: 12, color: "#5F5E5A", fontStyle: "italic", marginBottom: 4 }}>운영자: {isShowcase ? "저자 결론보다 문제제기에 집중" : "말보다 침묵의 장면이 핵심"}</div>
            <div style={{ fontSize: 12, color: "#2C2C2A", background: "#F3F2FC", padding: "6px 10px", borderRadius: 6 }}><span style={{ color: "#534AB7", fontWeight: 600, fontSize: 11 }}>대표</span> @닉A {isShowcase ? "3장 핵심은 결론이 아니라..." : "이건 대사보다 연출 의도를..."}</div>
            <MetaRow comments={36} upvotes={71} compact />
          </div>
          <SH>최신 이슈</SH>
          <RoomIssueCard title={isShowcase ? "4장 반전 구조 분석" : "이번 데이트 선택, 어떻게 읽히나"} opLine={isShowcase ? "이 장의 핵심은 반전 자체가 아닙니다" : "감정보다 전략으로 읽힙니다"} repShort={isShowcase ? "@닉B 오히려 이 반전이 저자의 진짜 의도..." : "@닉B 이건 호감보다 연출에 가까워..."} status="진행 중" comments={28} upvotes={53} noRepYet={false} />
          <RoomIssueCard title={isShowcase ? "5장 프리뷰" : "다음 회차 예고에서 궁금한 점"} opLine={isShowcase ? "여기서부터 논조가 바뀝니다" : "예고편 편집이 핵심"} repShort="" status="예정" comments={8} upvotes={19} noRepYet={false} />
        </>
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ padding: "24px 16px 16px", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "center", marginBottom: 16 }}>
          {[
            ...
            <div key={i} style={{ flex: 1, padding: "10px 4px", background: "#F9F9F6", borderRadius: 10, textAlign: "center" }}>
              <div style={{ fontSize: 18, fontWeight: 700, color: "#2C2C2A" }}>{s.val}</div>
      ...
      {[
        ...
        <div key={i} style={{ padding: "10px 16px", borderBottom: "1px solid #F1EFE8" }}>
          <div style={{ fontSize: 13, color: "#444441", marginBottom: 2 }}>{c.text}</div>
          <div style={{ fontSize: 11, color: "#B4B2A9" }}>→ {c.issue} · {c.time}</div>
        </div>
      ))}
      {/* 참여 중인 룸 */}
      <SH>참여 중인 룸</SH>
...
function MyPageScreen({ state }) {
  ...
  return (
    <div>
      ...
      <ProfileSection />
      <div style={{ display: "flex", borderBottom: "2px solid #EEEDEA" }}>{["룸", "이슈", "댓글"].map((t, i) => (<div key={t} style={{ flex: 1, padding: "12px 0", textAlign: "center", fontSize: 14, fontWeight: activeTab === i ? 600 : 400, color: activeTab === i ? "#534AB7" : "#888780", borderBottom: activeTab === i ? "2px solid #534AB7" : "none", marginBottom: -2 }}>{t}</div>))}</div>
...
function ProfileSection() {
  return (
    <div style={{ padding: "16px", display: "flex", alignItems: "center", gap: 12, borderBottom: "1px solid #EEEDEA" }}>
      ...
      <div style={{ flex: 1 }}><div style={{ fontSize: 16, fontWeight: 600, color: "#2C2C2A" }}>@김해석</div><div style={{ fontSize: 12, color: "#888780" }}>콘텐츠를 맥락으로 읽는 사람</div></div>
...
function RoomItem({ name, color, icon, action }) {
  ...
  return (<div style={{ padding: "10px 16px", display: "flex", alignItems: "center", gap: 10, borderBottom: "1px solid #F1EFE8" }}><div style={{ width: 32, height: 32, borderRadius: 8, background: c.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: c.t }}>{icon}</div><span style={{ fontSize: 14, color: "#2C2C2A", flex: 1 }}>{name}</span>{action && <span style={{ padding: "4px 10px", background: "#F1EFE8", borderRadius: 6, fontSize: 12, color: "#5F5E5A", fontWeight: 600 }}>{action}</span>}</div>);
}

function MyPageRoomTab() { return (<><SH>구독 중인 룸</SH><RoomItem name="나는 솔로 해석 룸" color="purple" icon="◈" /><RoomItem name="리벨북스 비문학 스터디" color="purple" icon="◈" /><SH>참여 중인 룸</SH><RoomItem name="흑백요리사 해석 룸" color="teal" icon="◈" /><SH>운영 중인 룸</SH><RoomItem name="테슬라 한국 실사용자 룸" color="amber" icon="★" action="운영하기" /></>); }

function SubTab({ tabs, active, onSelect }) { return (<div style={{ display: "flex", gap: 6, padding: "10px 16px" }}>{tabs.map(s => (<span key={s.k} onClick={() => onSelect(s.k)} style={{ padding: "5px 12px", borderRadius: 20, background: active === s.k ? "#2C2C2A" : "#F1EFE8", color: active === s.k ? "#fff" : "#888780", fontSize: 12, fontWeight: active === s.k ? 600 : 400, cursor: "pointer" }}>{s.l}</span>))}</div>); }
...
function MyPageIssueTab() { const [s, setS] = useState("scrap"); return (<><SubTab tabs={[{ k: "scrap", l: "스크랩" }, { k: "written", l: "작성" }, { k: "upvoted", l: "업보트" }]} active={s} onSelect={setS} />{s === "scrap" && (<><MI title="이번 데이트 선택, 어떻게 읽히나" room="나는 솔로 해석 룸" /><MI title="올해 테슬라 가격 전략 분석" room="테슬라 한국 실사용자 룸" /></>)}{s === "written" && <MI title="FSD 한국 도입, 올해 안 가능할까?" room="테슬라 한국 실사용자 룸" />}{s === "upvoted" && (<><MI title="이번 장의 핵심 질문은 무엇인가" room="리벨북스 비문학 스터디" /><MI title="이번 선택, 진짜 의도였을까?" room="나는 솔로 해석 룸" /></>)}</>); }
function MyPageCommentTab() { const [s, setS] = useState("scrap"); return (<><SubTab tabs={[{ k: "scrap", l: "스크랩" }, { k: "written", l: "작성" }, { k: "upvoted", l: "업보트" }]} active={s} onSelect={setS} />{s === "scrap" && <MC nick="@닉A" text="도입보다 과금 구조가 더 관건..." issue="FSD 한국 도입..." />}{s === "written" && (<><MC nick="나" text="이 장면은 편집보다 출연자 의도가 더 크다고 봅니다." issue="이번 데이트 선택..." /><MC nick="나" text="3장 후반부 논증 구조가 좀 약한 것 같습니다." issue="이번 장의 핵심..." /></>)}{s === "upvoted" && <MC nick="@닉B" text="이번 장면은 호감보다 편집 의도가 드러나는 순간..." issue="이번 선택, 진짜..." />}</>); }
      ...
      <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <RoomThumb size={64} />
            <div style={{ padding: "8px 16px", border: "1px dashed #D3D1C7", borderRadius: 8, fontSize: 13, color: "#888780", cursor: "pointer" }}>이미지 변경</div>
          </div>
        </div>
...
// ========= INFO PANEL =========
const SCREEN_INFO = {
  feed: { route: "/feed", purpose: "구독 룸들의 최신 이슈를 큰 카드로 소비하는 lean-back reading 피드", data: ["구독 Room 아바타 바", "큰 이슈 카드: 운영자 한 줄 + 본문 3줄 + 첨부 이미지 + 대표 댓글(최대 5줄)", "조회수/댓글수/업보트수"], cta: ["이슈 읽기", "룸 보기(룸명 탭)"], notes: "대표 댓글 지정 이슈만 노출. 첨부 이미지가 있는 이슈는 카드에 이미지 표시. 대표 댓글 5줄 제한. 조회수 표시." },
  issues: { route: "/issues", purpose: "공개 이슈를 빠르게 스캔하는 탐색 목록", data: ["작은 이슈 카드: 제목+운영자 한 줄+대표 댓글 1줄", "조회수/댓글수/업보트수"], cta: ["이슈 읽기", "룸 보기"], notes: "대표 댓글 지정 이슈 우선. 진행 중은 배지 미표시, 예정/종료만." },
  rooms: { route: "/rooms", purpose: "Visitor가 공개 룸을 발견하는 진입 화면", data: ["공개 Room + 대표 이미지", "읽기 중심/참여 가능 배지"], cta: ["구독하기", "룸 상세"], notes: "Visitor 진입점. 룸 대표 이미지 표시." },
  room: { route: "/rooms/:slug", purpose: "룸의 결을 느끼고 구독을 결정 (역할별 레이아웃 변화)", data: ["룸 대표 이미지+정보", "대표 반응(V/S)", "이슈 목록"], cta: ["구독", "운영하기(Op)", "⋯ 멤버 가입(확장형)", "건의/제보(Member)"], notes: "Member: 소개+대표반응 접기, 건의/제보 채널 노출, 진행 중 이슈 위로(미지정 이슈 보라 점)." },
  issue: { route: "/rooms/:slug/issues/:id", purpose: "이슈 본문 + 대표 댓글(1개) + 전체 댓글 소비", data: ["이슈 전체", "운영자 한 줄", "작성일", "조회수", "대표 댓글(1개)", "전체 댓글+대댓글"], cta: ["업보트", "스크랩", "신고", "댓글 작성(+이미지)", "정렬"], notes: "Visitor→구독 유도, Subscriber→멤버 신청, Member/Op→댓글 입력. 조회수 표시." },
  mypage: { route: "/me", purpose: "개인 활동과 룸 관계 허브", data: ["프로필(→프로필 페이지 링크)", "룸/이슈/댓글 탭"], cta: ["룸 보기", "운영하기", "프로필 보기"], notes: "프로필 섹션에서 상세 프로필 페이지로 이동 가능." },
  profile: { route: "/users/:id", purpose: "개인의 활동과 업적을 한눈에 보여주는 프로필 페이지", data: ["프로필 정보", "활동 통계(작성 댓글/받은 좋아요/대표 선정/구독 룸)", "대표 선정 댓글", "최근 댓글", "참여 룸"], cta: ["프로필 편집(내 프로필)", "댓글 보기"], notes: "내 프로필/다른 사용자 프로필 전환 가능. 대표 선정 횟수가 핵심 업적 지표." },
  "room-settings": { route: "/operator/rooms/:id/settings", purpose: "운영자가 룸 관리", data: ["룸 대표 이미지 설정", "룸 이름/설명", "유형/공개 상태", "멤버"], cta: ["이미지 변경", "저장", "멤버 관리"], notes: "룸 대표 이미지 업로드 기능 추가." },
  "issue-publish": { route: "/operator/rooms/:id/issues/new", purpose: "이슈 발행/수정", data: ["제목/운영자 한 줄/질문/본문/첨부", "상태/대표/스포일러"], cta: ["발행", "수정", "삭제"], notes: "운영자 한 줄 복원." },
