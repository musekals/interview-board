...
const STATES = {
  ...
  // [v8.1] 멤버모집 → 초대 참여 가능. Member(공유가능) = 공유용 링크 활성
  room: [
    "초대참여가능-Visitor", "초대참여가능-Subscriber",
    "초대참여가능-Member", "초대참여가능-Member(공유가능)",
    "초대참여가능-Operator",
    "모집안함-Visitor", "모집안함-Subscriber",
    "모집안함-Member", "모집안함-Operator",
  ],
  // [v8.1] 이슈 상세에도 모집 상태 축 반영
  issue: [
    "Visitor", "Subscriber(초대참여가능)", "Subscriber(모집안함)",
    "Member/Operator", "대표 댓글 없음",
  ],
  mypage: ["룸 탭", "이슈 탭", "댓글 탭", "구독 없음(Empty)"],
  profile: ["내 프로필", "다른 사용자 프로필"],
  // [v8.1] 중지/모집종료 상태 추가
  "invite-landing": ["유효", "만료·사용완료", "중지·모집종료", "이미 멤버"],
  // [v8.1] 공유용 링크 상태 분기
  "room-settings": ["공유링크-없음", "공유링크-활성", "공유링크-소진", "공유링크-만료", "공유링크-중지"],
...
function FeedScreen({ state }) {
  ...
  return (<div><div style={{ padding: "14px 16px 8px", fontSize: 20, fontWeight: 700, color: "#2C2C2A" }}>내 피드</div><SubscribedRoomBar /><BigIssueCard room="테슬라 한국 실사용자 룸" title="FSD 한국 도입, 올해 안 가능할까?" opLine="일정보다 가격/체감이 핵심이라 봅니다" body="이번 FSD 도입 논의에서 가장 핵심적인 부분은 일정 자체가 아니라 가격 구조와 실제 체감 성능입니다." repNick="@닉A" repComment="도입보다 과금 구조가 더 관건이라는 이야기가 많은데, 실제로 한국 시장에서 FSD 월정액 모델이 성립하려면 OTA 체감 품질이 먼저 올라와야 합니다." status="진행 중" comments={24} upvotes={47} views={312} hasImage /><BigIssueCard room="나는 솔로 해석 룸" title="이번 선택, 진짜 의도였을까?" opLine="행동보다 편집 포인트에 주목해주세요" body="이번 장면은 표면적인 대사보다도 카메라가 담은 시선의 방향과 편집 타이밍에 핵심이 있습니다." repNick="@닉B" repComment="이번 장면은 호감 표현이 아니라 편집 의도가 드러나는 순간이라고 봅니다." status="진행 중" comments={31} upvotes={62} views={487} hasImage /><BigIssueCard room="리벨북스 비문학 스터디" title="이번 장의 핵심 질문은 무엇인가" opLine="저자 결론보다 문제제기에 집중합시다" body="저자가 3장에서 제기하는 문제의식은 결론 부분보다 오히려 초반의 질문 자체에 더 큰 무게가 있습니다." repNick="@닉E" repComment="오히려 이 대목이 저자의 핵심 논점입니다." status="종료" comments={18} upvotes={35} views={198} /></div>);
}
...
// [v8.1] 룸 목록 — 멤버 신청 제거, 배지 카피 변경
function RoomListScreen({ state }) {
  const rooms = [
    { name: "나는 솔로 해석 룸", desc: "감정보다 맥락과 편집으로 봅니다", inviteOpen: false, issues: 12, subs: 84 },
    { name: "테슬라 한국 실사용자 룸", desc: "루머보다 해석, 드립보다 이유", inviteOpen: true, issues: 18, subs: 156 },
    { name: "리벨북스 비문학 스터디", desc: "저자의 질문을 함께 읽습니다", inviteOpen: false, issues: 8, subs: 47 },
  ];
  return (<div><div style={{ padding: "14px 16px 4px", fontSize: 20, fontWeight: 700, color: "#2C2C2A" }}>공개 룸</div><div style={{ padding: "4px 16px 12px", fontSize: 13, color: "#888780", lineHeight: 1.5 }}>결 맞는 룸을 구독하고, 이슈와 반응을 내 피드로 모아보세요</div>{rooms.map((r, i) => (<div key={i} style={{ padding: "14px 16px", borderBottom: "1px solid #EEEDEA" }}><div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}><RoomThumb size={44} /><div style={{ flex: 1 }}><div style={{ fontSize: 15, fontWeight: 600, color: "#2C2C2A" }}>{r.name}</div><div style={{ fontSize: 12, color: "#888780" }}>{r.desc}</div></div></div><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>{r.inviteOpen && <Badge text="초대 참여 가능" color="teal" />}<span style={{ fontSize: 11, color: "#B4B2A9" }}>이슈 {r.issues}개 · 구독자 {r.subs}명</span></div><div style={{ flex: 1, padding: "8px 0", background: "#534AB7", color: "#fff", borderRadius: 8, fontSize: 13, fontWeight: 600, textAlign: "center" }}>{state === "Subscriber" ? "구독 중 ✓" : "구독하기"}</div></div>))}</div>);
}

// [v8.1] 룸 상세 — invite-first
function RoomScreen({ state }) {
  const isShareable = state === "초대참여가능-Member(공유가능)";
  const cleanState = isShareable ? "초대참여가능-Member" : state;
  const parts = cleanState.split("-");
  const inviteOpen = parts[0] === "초대참여가능";
  ...
  return (<div>
    ...
    <div style={{ padding: "16px 16px 12px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}><RoomThumb size={52} /><div style={{ flex: 1 }}><div style={{ fontSize: 19, fontWeight: 700, color: "#2C2C2A", marginBottom: 2 }}>나는 솔로 해석 룸</div><div style={{ display: "flex", alignItems: "center", gap: 6 }}>{inviteOpen && <Badge text="초대 참여 가능" color="teal" />}<span style={{ fontSize: 11, color: "#B4B2A9" }}>구독자 84명</span></div></div></div>
      <div style={{ fontSize: 13, color: "#5F5E5A", marginBottom: 10 }}>감정보다 맥락과 편집으로 봅니다</div>
      {isMember ? <div style={{ fontSize: 12, color: "#B4B2A9", padding: "6px 0", marginBottom: 8, cursor: "pointer" }}>▸ 룸 소개 보기</div> : <div style={{ fontSize: 13, color: "#444441", background: "#F9F9F6", padding: "12px 14px", borderRadius: 10, lineHeight: 1.6, marginBottom: 12, whiteSpace: "pre-line" }}><div style={{ fontWeight: 600, fontSize: 12, color: "#888780", marginBottom: 6 }}>이 방은 이렇게 봅니다</div>{"• 장면의 의도와 편집 포인트를 같이 봅니다\n• 한 줄 반응보다 이유 있는 댓글을 중시합니다"}</div>}
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        {isShareable && <div style={{ padding: "11px 14px", background: "#E1F5EE", color: "#0F6E56", borderRadius: 10, fontSize: 13, fontWeight: 600, border: "1.5px solid #5DCAA5" }}>지인에게 공유</div>}
        {isMember && <div style={{ padding: "11px 14px", border: "1px solid #D3D1C7", color: "#888780", borderRadius: 10, fontSize: 13 }}>멤버 탈퇴</div>}
        {isOp && <div style={{ padding: "11px 16px", background: "#F1EFE8", color: "#5F5E5A", borderRadius: 10, fontSize: 13, fontWeight: 600 }}>운영하기</div>}
      </div>

      {inviteOpen && !isMember && !isOp && (
    ...
    {isMember && <div style={{ margin: "0 16px 8px", padding: "10px 14px", background: "#F9F9F6", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "space-between" }}><span style={{ fontSize: 12, color: "#5F5E5A" }}>운영자에게 건의/제보</span><span style={{ fontSize: 12, color: "#534AB7", fontWeight: 600 }}>보내기 →</span></div>}
    ...
    {isMember ? (<>
      ...
      <SH>대표 이슈</SH>
      <div style={{ margin: "0 16px 8px", padding: 14, border: "1.5px solid #AFA9EC", borderRadius: 12, background: "#FCFCFA" }}><div style={{ fontSize: 14, fontWeight: 600, color: "#2C2C2A", marginBottom: 4 }}>이번 회차의 진짜 전환점은 어디였나</div><div style={{ fontSize: 12, color: "#5F5E5A", fontStyle: "italic", marginBottom: 4 }}>운영자: 말보다 침묵의 장면이 핵심</div><div style={{ fontSize: 12, color: "#2C2C2A", background: "#F3F2FC", padding: "6px 10px", borderRadius: 6 }}><span style={{ color: "#534AB7", fontWeight: 600, fontSize: 11 }}>대표</span> @닉A 이건 대사보다 연출 의도를...</div><MetaRow comments={36} upvotes={71} compact /></div>
      <SH>최신 이슈</SH>
...
// [v8.1] 이슈 상세 — 모집 상태 축 반영
function IssueDetailScreen({ state }) {
  const canComment = state === "Member/Operator";
  const isSubInviteOpen = state === "Subscriber(초대참여가능)";
  const isSubClosed = state === "Subscriber(모집안함)";
  ...
  return (<div>
    ...
    <div style={{ padding: "12px 16px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: "1px solid #EEEDEA" }}><div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 14px", borderRadius: 20, border: "1.5px solid #D3D1C7", fontSize: 13, fontWeight: 600, color: "#5F5E5A" }}>★ 업보트 53</div><div style={{ padding: "6px 10px", borderRadius: 20, border: "1.5px solid #D3D1C7", fontSize: 12, color: "#888780" }}>⊞ 스크랩</div><div style={{ padding: "6px 10px", borderRadius: 20, border: "1.5px solid #D3D1C7", fontSize: 12, color: "#B4B2A9" }}>⚑ 신고</div></div><div style={{ fontSize: 12, color: "#B4B2A9" }}>댓글 28 · 조회 421</div></div>
    </div>
    ...
    <div style={{ padding: "12px 16px", borderTop: "1px solid #EEEDEA", background: "#FAFAF8" }}>
      {canComment ? (
        ...
        /* [v8.1] 모집 안 하는 룸의 Subscriber */
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 13, color: "#888780", marginBottom: 6 }}>댓글은 멤버만 작성할 수 있어요</div>
          <div style={{ fontSize: 12, color: "#B4B2A9" }}>현재 이 룸은 새 멤버를 받고 있지 않습니다</div>
        </div>
      ) : (
...
// ===== [v8.1] 룸 설정 — 공유링크 상태 분기 + 1:1 생성→복사 =====
function RoomSettingsScreen({ state }) {
  // 공유용 링크 상태
  const shareState = state ? state.replace("공유링크-", "") : "없음";

  // 1:1 초대 링크: 생성 후 복사 상호작용
  ...
  return (<div>
    ...
    <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div>
        ...
        <div style={{ fontSize: 11, color: "#B4B2A9", marginTop: 4 }}>닫으면 모든 활성 초대 링크가 자동 중지됩니다</div>
      </div>

      {/* [v8.1] 1:1 초대 링크 — 생성→복사 상호작용 */}
      <div style={{ padding: "14px", background: "#F9F9F6", borderRadius: 12, border: "1px solid #EEEDEA" }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "#2C2C2A", marginBottom: 2 }}>1:1 초대 링크</div>
        <div style={{ fontSize: 11, color: "#888780", marginBottom: 10 }}>1회용 · 7일 만료 · 버튼을 누를 때마다 새 링크 생성</div>
        {!generated ? (
          <div onClick={() => setGenerated(true)} style={{ padding: "9px 0", background: "#534AB7", color: "#fff", borderRadius: 8, fontSize: 13, fontWeight: 600, textAlign: "center", cursor: "pointer" }}>링크 생성</div>
        ) : (
          <div>
            <div style={{ padding: "8px 12px", background: "#F3F2FC", borderRadius: 8, marginBottom: 8 }}>
              <div style={{ fontSize: 11, color: "#888780", marginBottom: 3 }}>생성된 링크</div>
              <div style={{ fontSize: 12, color: "#534AB7", fontFamily: "monospace", wordBreak: "break-all" }}>issueroom.kr/invite/x9k2m7</div>
            </div>
            <div style={{ display: "flex", gap: 6 }}>
              <div style={{ flex: 1, padding: "8px 0", background: "#534AB7", color: "#fff", borderRadius: 8, fontSize: 12, fontWeight: 600, textAlign: "center" }}>링크 복사</div>
              <div onClick={() => setGenerated(false)} style={{ padding: "8px 12px", border: "1px solid #D3D1C7", color: "#888780", borderRadius: 8, fontSize: 12, textAlign: "center", cursor: "pointer" }}>새로 생성</div>
            </div>
          </div>
        )}
      </div>

      {/* [v8.1] 공유용 초대 링크 — 상태별 분기 */}
      <div style={{ padding: "14px", background: "#F8FEFB", borderRadius: 12, border: "1px solid #D7EBE4" }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "#2C2C2A", marginBottom: 2 }}>공유용 초대 링크</div>
        <div style={{ fontSize: 11, color: "#5F5E5A", marginBottom: 10 }}>멤버가 지인에게 전달할 수 있는 링크입니다</div>

        {shareState === "없음" && (
          <div>
            <div style={{ fontSize: 12, color: "#B4B2A9", marginBottom: 8 }}>아직 공유용 링크가 없습니다</div>
            <div style={{ padding: "9px 0", background: "#0F6E56", color: "#fff", borderRadius: 8, fontSize: 13, fontWeight: 600, textAlign: "center" }}>공유용 링크 열기</div>
            <div style={{ fontSize: 11, color: "#B4B2A9", marginTop: 6 }}>5회용 · 7일 규칙으로 생성됩니다</div>
          </div>
        )}
        ...
        {shareState === "활성" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
              <Badge text="활성 중" color="teal" />
              <span style={{ fontSize: 11, color: "#B4B2A9" }}>사용: 2/5 · 만료: 3일 후</span>
            </div>
            <div style={{ padding: "8px 12px", background: "#E1F5EE", borderRadius: 8, marginBottom: 8 }}>
              <div style={{ fontSize: 12, color: "#0F6E56", fontFamily: "monospace", wordBreak: "break-all" }}>issueroom.kr/invite/share-p3q8</div>
            </div>
            <div style={{ display: "flex", gap: 6 }}>
              <div style={{ flex: 1, padding: "8px 0", background: "#0F6E56", color: "#fff", borderRadius: 8, fontSize: 12, fontWeight: 600, textAlign: "center" }}>링크 복사</div>
              <div style={{ padding: "8px 12px", border: "1px solid #D3D1C7", color: "#888780", borderRadius: 8, fontSize: 12, textAlign: "center" }}>중지</div>
            </div>
            <div style={{ fontSize: 11, color: "#B4B2A9", marginTop: 6 }}>활성 상태에서만 멤버에게 &quot;지인에게 공유&quot; 버튼이 보입니다</div>
          </div>
        )}
        ...
        {shareState === "소진" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
              <Badge text="사용 완료" color="gray" />
              <span style={{ fontSize: 11, color: "#B4B2A9" }}>5/5 사용됨</span>
            </div>
            <div style={{ fontSize: 12, color: "#5F5E5A", marginBottom: 8 }}>모든 사용 횟수가 소진되었습니다</div>
            <div style={{ padding: "9px 0", background: "#0F6E56", color: "#fff", borderRadius: 8, fontSize: 13, fontWeight: 600, textAlign: "center" }}>새 링크 열기</div>
          </div>
        )}
        ...
        {shareState === "만료" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
              <Badge text="만료됨" color="amber" />
              <span style={{ fontSize: 11, color: "#B4B2A9" }}>7일 경과</span>
            </div>
            <div style={{ fontSize: 12, color: "#5F5E5A", marginBottom: 8 }}>링크가 만료되어 더 이상 사용할 수 없습니다</div>
            <div style={{ padding: "9px 0", background: "#0F6E56", color: "#fff", borderRadius: 8, fontSize: 13, fontWeight: 600, textAlign: "center" }}>새 링크 열기</div>
          </div>
        )}
        ...
        {shareState === "중지" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
              <Badge text="중지됨" color="coral" />
              <span style={{ fontSize: 11, color: "#B4B2A9" }}>운영자가 중지함</span>
            </div>
            <div style={{ fontSize: 12, color: "#5F5E5A", marginBottom: 8 }}>중지된 링크로는 가입할 수 없습니다</div>
            <div style={{ padding: "9px 0", background: "#0F6E56", color: "#fff", borderRadius: 8, fontSize: 13, fontWeight: 600, textAlign: "center" }}>새 링크 열기</div>
          </div>
        )}
      </div>
...
// ===== [v8.1] 초대 랜딩 — 중지·모집종료 상태 + 초대자 표시 =====
function InviteLandingScreen({ state }) {
  if (state === "만료·사용완료") return (<div style={{ padding: "60px 24px", textAlign: "center" }}><div style={{ width: 64, height: 64, borderRadius: "50%", background: "#F1EFE8", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, margin: "0 auto 16px", color: "#B4B2A9" }}>✕</div><div style={{ fontSize: 18, fontWeight: 700, color: "#2C2C2A", marginBottom: 8 }}>이 초대는 만료되었어요</div><div style={{ fontSize: 13, color: "#888780", lineHeight: 1.6, marginBottom: 24 }}>이 초대 링크는 만료되었거나<br />이미 사용된 링크입니다.</div><div style={{ display: "inline-block", padding: "11px 24px", background: "#534AB7", color: "#fff", borderRadius: 10, fontSize: 14, fontWeight: 600 }}>룸을 구독해 둘러보기</div></div>);

  // [v8.1] 중지·모집종료
  if (state === "중지·모집종료") return (<div style={{ padding: "60px 24px", textAlign: "center" }}><div style={{ width: 64, height: 64, borderRadius: "50%", background: "#FAEEDA", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, margin: "0 auto 16px", color: "#854F0B" }}>⏸</div><div style={{ fontSize: 18, fontWeight: 700, color: "#2C2C2A", marginBottom: 8 }}>지금은 참여할 수 없어요</div><div style={{ fontSize: 13, color: "#888780", lineHeight: 1.6, marginBottom: 24 }}>이 초대 링크는 운영자에 의해 중지되었거나,<br />현재 이 룸이 새 멤버를 받고 있지 않습니다.</div><div style={{ display: "inline-block", padding: "11px 24px", background: "#534AB7", color: "#fff", borderRadius: 10, fontSize: 14, fontWeight: 600 }}>룸을 구독해 둘러보기</div></div>);
  ...
  return (<div>
    <div style={{ padding: "32px 24px 20px", textAlign: "center", background: "linear-gradient(180deg, #F3F2FC 0%, #fff 100%)" }}>
      ...
      <div style={{ fontSize: 13, color: "#888780", marginBottom: 6 }}>감정보다 맥락과 편집으로 봅니다</div>
      {/* [v8.1] 초대한 사람 표시 */}
      <div style={{ fontSize: 12, color: "#AFA9EC", marginBottom: 16 }}>@닉A 님이 이 룸으로 초대했어요</div>
    </div>
...
// ===== INFO PANEL =====
const SCREEN_INFO = {
  feed: { route: "/feed", purpose: "구독 룸들의 최신 이슈를 큰 카드로 소비하는 피드", data: ["구독 Room 아바타 바", "큰 이슈 카드: 운영자 한 줄 + 본문 + 대표 댓글", "조회수/댓글수/업보트수"], cta: ["이슈 읽기", "룸 보기"], notes: "v6과 동일." },
  issues: { route: "/issues", purpose: "공개 이슈를 빠르게 스캔하는 탐색 목록", data: ["작은 이슈 카드"], cta: ["이슈 읽기", "룸 보기"], notes: "v6과 동일." },
  rooms: { route: "/rooms", purpose: "공개 룸 발견 화면", data: ["공개 Room + 초대 참여 가능 배지", "이슈 수/구독자 수"], cta: ["구독하기"], notes: "[v8.1] v6의 '멤버 신청' 버튼 제거. 구독 CTA만. '멤버 모집 중' 배지 → '초대 참여 가능'으로 변경." },
  room: { route: "/rooms/:slug", purpose: "룸의 결을 느끼고 구독을 결정하는 페이지", data: ["룸 정보", "대표 이슈", "이슈 목록"], cta: ["구독", "지인에게 공유(Member, 공유용 링크 활성 시)", "멤버 탈퇴(Member)", "운영하기(Op)"], notes: "[v8.1] v6의 '멤버 신청' 제거. Visitor/Subscriber에게 '참여는 초대 링크로 열립니다' 안내(초대 참여 가능 룸만). Member(공유가능): 공유용 링크 활성 시에만 '지인에게 공유' 버튼, 비활성이면 숨김. '멤버 모집 중' → '초대 참여 가능' 배지." },
  issue: { route: "/rooms/:slug/issues/:id", purpose: "이슈 본문 + 대표 댓글 + 전체 댓글", data: ["이슈 전체", "대표 댓글(1개)", "전체 댓글+대댓글"], cta: ["업보트", "스크랩", "댓글 작성"], notes: "[v8.1] v6의 'Subscriber(구독만)' → 2개로 분리. 초대 참여 가능 룸: '참여는 초대 링크로 열립니다' + '이 룸에서 참여 방법 보기'. 모집 안 하는 룸: '현재 이 룸은 새 멤버를 받고 있지 않습니다'. Visitor: 구독 유도(v6 동일)." },
  mypage: { route: "/me", purpose: "개인 활동과 룸 관계 허브", data: ["프로필", "룸/이슈/댓글 탭"], cta: ["룸 보기", "운영하기"], notes: "v6과 동일. A릴리즈에서 초대 탭 미추가." },
  profile: { route: "/users/:id", purpose: "활동과 업적 프로필", data: ["통계", "대표 선정 댓글", "참여 룸"], cta: ["편집"], notes: "v6과 동일." },
  "invite-landing": { route: "/invite/:token", purpose: "초대 링크로 진입한 사용자에게 룸의 결을 보여주고 가입 유도", data: ["초대한 사람(@닉)", "룸 소개", "대표 이슈 미리보기"], cta: ["가입하고 참여하기"], notes: "[v8.1] 4개 상태: 유효(초대자 표시+가입 CTA), 만료·사용완료(만료/사용된 링크→구독 유도), 중지·모집종료(운영자 중지 또는 모집 닫힘→구독 유도), 이미 멤버(이슈 보기). 비회원은 회원가입 후 자동 멤버+자동 구독." },
  "room-settings": { route: "/operator/rooms/:id/settings", purpose: "운영자가 룸을 관리", data: ["룸 정보", "멤버 모집 (닫힘/열림)", "1:1 초대 링크", "공유용 초대 링크(상태별)", "공개 상태", "멤버 목록"], cta: ["1:1 링크 생성→복사(일회성)", "공유용 링크 열기/복사/중지", "저장"], notes: "[v8.1] v6의 '멤버 신청 받기' 토글 → '멤버 모집 닫힘/열림'. 닫으면 모든 활성 링크 자동 중지. 1:1 초대: 버튼→링크 카드+복사 버튼 즉시 노출, 상태 관리 없음. 공유용 초대: 5개 상태별 분기(없음/활성/소진/만료/중지). 활성 시 멤버에 '지인에게 공유' 노출. 별도 초대 관리 대시보드 없음." },
